...
export function getViewportMode(width: number, height: number): ViewportMode {
	...
	if (width < 76 || height < 25) return "compact";
...
function petArt(name: string): string {
	return `    /\\_/\\
   ( o.o )
    > ^ <
...
export class AppView {
	...
	constructor(
		...
		this.sidebar.add(
			new TextRenderable(renderer, { content: "Hackagotchi", fg: theme.muted }),
		);
		...
		main.add(this.habitat);
		main.add(this.condition);
		main.add(this.stats);
		...
		for (const row of this.shopRows) main.add(row);
		...
		this.rail.add(
			new TextRenderable(renderer, { content: "Details", fg: theme.muted }),
		);
		...
		renderer.root.onSizeChange = () => this.applyViewport();
	...
	update(snapshot: AppSnapshot): void {
		...
		this.habitat.visible = snapshot.page === "home";
		this.condition.visible = snapshot.page === "home";
		this.stats.visible = snapshot.page === "home";
		if (pet) {
			this.habitat.content = petArt(pet.name);
			this.condition.content = pet.isAlive
				? "Settled in and alert"
				: "Needs your attention!!";
			this.condition.fg = pet.isAlive ? theme.green : theme.red;
			this.stats.content = `${meter("Hunger", pet.hunger)}\n${meter("Happiness", pet.happiness)}\n${meter("Energy", pet.energy)}`;
		} else {
			this.habitat.content =
				"    .--.\n   (    )\n    `--'\n\npsst... press c to hatch me!";
			this.condition.content = "No companion yet";
			this.condition.fg = theme.yellow;
			this.stats.content = "Create a pet to begin tracking its condition.";
		}
		for (const [index, row] of this.shopRows.entries()) {
			...
			row.content = `${selected ? "›" : " "} ${index + 1}. ${entry.item.name}  ${entry.item.price} coins\n    ${entry.item.description}`;
		...
		this.detail.content = selected
			? `${selected.item.name}\n\n${selected.item.description}\n\nPrice: ${selected.item.price} coins\n\nPress Enter to add it to your inventory. The shop refreshes each day.`
	...
	private applyViewport(): void {
		...
		this.habitat.height = mode === "compact" ? 4 : 6;
	}
